...
const uint16_t PROGMEM numpad_comma_combo[] = {KC_P0, KC_PDOT, COMBO_END};
combo_t key_combos[] = {
    [COMBO_NUMPAD_COMMA] = COMBO(numpad_comma_combo, KC_COMM)
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed) {
        switch(keycode) {
            case KC_F15: // 'X' playstation button
                layer_invert(PROTO);
                return false;
        }
    }
